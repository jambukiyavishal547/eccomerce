...
def validate_cnew_password(request):
	...
	if newpassword==cnewpassword:
		flag=True
	data={
	'is_taken':User.objects.filter(password=cnewpassword).exists()
	}
	return JsonResponse(data)

def validate_product_name(request):
	product_name=request.GET.get('products_name')
	seller=User.objects.get(email=request.session['email'])
	data={
	'is_taken':Product.objects.filter(product_name=product_name,seller=seller).exists()
	}
	return JsonResponse(data) 
		...
		try:
			...
			if user.password==request.POST['password']:
				if user.usertype=="buyer":
					...
					request.session['new-password']=user.new-password
